...
const {getBusyState, canStartJob, startJob, endJob} = require('./busyState');

let menuName = "";

// 중복동작 제한 헬퍼 추가
async function runBusyJob(jobType, jobId, fn) {
    const guard = canStartJob(jobType);
    ...
    return runBusyJob(
        ...
        async () => {
            try {
                ...
                const orderData = data.orderList;

                // 전체 메뉴 조회
                const menu = await allProduct();
                if (!menu || !menu.Items) {
                    throw new Error("전체 메뉴 조회에 실패했습니다.");
                }
                ...
                if (menuData.length > 0) {
                    log.info("[주문 시작] 주문 제조를 시작합니다.");
                    log.info("[주문 목록]", orderData);

                    await processQueue(orderData, menuData);
                    await useWash(orderData);
                } else {
                    log.warn("[주문 경고] 메뉴 정보 또는 주문 정보가 없습니다.");
                }
            } catch (error) {
                log.error("[주문 오류] 주문 시작 중 오류 발생:", error.message);
                throw error;
            } finally {
...
const adminCupOrder = async (recipe) => {
    return runBusyJob(
        "adminCup",
        `admin-cup-${Date.now()}`,
        async () => {
...
const adminUseWash = async (recipe) => {
    return runBusyJob(
        "adminWash",
        `admin-wash-${Date.now()}`,
        async () => {
            ...
            log.info('////////--------------- 어드민 세척 요청 --------------------//////');
            log.info('////////--------------- 어드민 세척 요청 --------------------//////');
            let washData = recipe.data;  // 세척 데이터
